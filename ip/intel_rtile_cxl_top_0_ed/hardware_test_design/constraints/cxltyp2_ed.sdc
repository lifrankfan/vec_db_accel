...
#-----------------------------------------------
# Custom AFU CDC Constraints
#-----------------------------------------------
# Set false paths for asynchronous clock domain crossings in cust_afu_wrapper
# These signals are properly synchronized using cdc_sync_flop modules

# Get the actual clock objects for the CSR and AXI domains
set csr_clocks [get_clocks -nowarn *ip2csr_avmm_clk*]
set axi_clocks [get_clocks -nowarn *ip2hdm_clk*]

# From CSR clock domain to AXI clock domain (configuration registers)
set csr_to_axi_registers [get_registers -nowarn {*cust_afu_wrapper_inst|test_case_cdc_inst|s_data* *cust_afu_wrapper_inst|start_proc_cdc_inst|s_data* *cust_afu_wrapper_inst|seed_cdc_inst|s_data* *cust_afu_wrapper_inst|num_request_cdc_inst|s_data* *cust_afu_wrapper_inst|addr_range_cdc_inst|s_data*}]
if {[get_collection_size $csr_to_axi_registers] > 0 && [get_collection_size $csr_clocks] > 0} {
    set_false_path -from $csr_clocks -to $csr_to_axi_registers
}

# From AXI clock domain to CSR clock domain (status/counter registers)  
set axi_to_csr_registers [get_registers -nowarn {*cust_afu_wrapper_inst|delay_out_cdc_inst|s_data* *cust_afu_wrapper_inst|resp_out_cdc_inst|s_data* *cust_afu_wrapper_inst|addr_cnt_out_cdc_inst|s_data* *cust_afu_wrapper_inst|data_cnt_out_cdc_inst|s_data* *cust_afu_wrapper_inst|resp_cnt_out_cdc_inst|s_data* *cust_afu_wrapper_inst|id_cnt_out_cdc_inst|s_data* *cust_afu_wrapper_inst|id_cnt_1_out_cdc_inst|s_data*}]
if {[get_collection_size $axi_to_csr_registers] > 0 && [get_collection_size $axi_clocks] > 0} {
    set_false_path -from $axi_clocks -to $axi_to_csr_registers
}

# Set false paths for registered outputs before CDC (these are held stable)
set axi_register_outputs [get_registers -nowarn {*cust_afu_wrapper_inst|delay_out_reg* *cust_afu_wrapper_inst|resp_out_reg* *cust_afu_wrapper_inst|addr_cnt_out_reg* *cust_afu_wrapper_inst|data_cnt_out_reg* *cust_afu_wrapper_inst|resp_cnt_out_reg* *cust_afu_wrapper_inst|id_cnt_out_reg* *cust_afu_wrapper_inst|id_cnt_1_out_reg*}]
if {[get_collection_size $axi_register_outputs] > 0 && [get_collection_size $csr_clocks] > 0} {
    set_false_path -from $axi_register_outputs -to $csr_clocks
}

# Apply CDC-specific timing constraints for synchronizer flip-flops
set all_cdc_sync_registers [get_registers -nowarn {*cust_afu_wrapper_inst|*_cdc_inst|s_data*}]
if {[get_collection_size $all_cdc_sync_registers] > 0} {
    # Set maximum skew between bits in the same bus to minimize metastability window
    set_max_skew -to $all_cdc_sync_registers 0.100
}
